...
def popUpClose(driver):
    for i in range(2):
        try:
            ...
            logging.info("Closed Pop up found")
        except:
...
def takeBackSession(driver):
    global numberRetryCalls
    iframeHandler(driver, title='Home')
    try:

        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//span[normalize-space()='Take back session']"))).click()
        numberRetryCalls = 3
        #sessionStart(driver, file, vac_device_id)
    except :
        logging.info("No need to take back session")
    iframeHandler(driver, default=True)
